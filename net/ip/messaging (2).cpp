...
{
	boost::thread * t = new boost::thread (boost::bind (handler, this));
...
{
	listen_async_internal (handler, broadcast_address, port, cycle);
}

void LIB::network::mpi::listen_async_internal (void (* handler) (std::string), const std::string & broadcast_address, const unsigned short int & port, const bool & cycle, unsigned long int hash)
{
	/*boost::thread h = */new boost::thread (boost::bind (& LIB::network::mpi::listen_accept, this, handler, broadcast_address, port-, cycle, hash));
}

void LIB::network::mpi::listen_accept (void (* handler) (std::string), const std::string & broadcast_address, const unsigned short int & port, const bool & cycle, unsigned long int hash, unsigned long long int & handler_index)
{
...
//	else
//	{
//		// "hash" and "handler_index" should change simultaneously: they should both be "0" or they should both not be "0".
//		if (handler_index != 0)
//		{
//			io_service = handlers [hash] [handler_index];
//		}
//		
//		handlers [hash].set (handler_index, io_service);
//	}
...
//		
		if (cycle)
		{
			...
			listen_async_internal (handler, broadcast_address, port, cycle, hash, handler_index);
		}
