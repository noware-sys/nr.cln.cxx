...
{
	//std::cout << (addr & 0xFF) << '.' << (addr >> 8 & 0xFF) << '.' << (addr >> 16 & 0xFF) << '.' << (addr >> 24 & 0xFF) << std::endl;
	//std::cout << (LIB::containers::NAME_V) (addr & 0xFF) << '.' << (LIB::containers::NAME_V) (addr >> 8 & 0xFF) << '.' << (LIB::containers::NAME_V) (addr >> 16 & 0xFF) << '.' << (LIB::containers::NAME_V) (addr >> 24 & 0xFF) << std::endl;
	//std::cout << (LIB::containers::NAME_V) (addr & 0xFF) + '.' + (LIB::containers::NAME_V) (addr >> 8 & 0xFF) + '.' + (LIB::containers::NAME_V) (addr >> 16 & 0xFF) + '.' + (LIB::containers::NAME_V) (addr >> 24 & 0xFF) << std::endl;
	//std::cout << (LIB::containers::NAME_V) (addr & 0xFF) + '.' << std::endl;
	//std::cout << LIB::tools::string (addr & 0xFF) + '.' + LIB::tools::string (addr >> 8 & 0xFF) + '.' + LIB::tools::string (addr >> 16 & 0xFF) + '.' + LIB::tools::string (addr >> 24 & 0xFF) << std::endl;
	//LIB::tools::string
	return LIB::tools::string (addr & 0xFF) + '.' + LIB::tools::string (addr >> 8 & 0xFF) + '.' + LIB::tools::string (addr >> 16 & 0xFF) + '.' + LIB::tools::string (addr >> 24 & 0xFF);
	...
	if (sd > 0)
	{
		...
		if (ioctl (sd, SIOCGIFCONF, & ifc) == 0)
		{
			...
			for (i = 0; i < ifc_num; ++ i)
			{
				...
				{
					std::cout << "Unknown family:[" << family << ']' << std::endl;
					continue;
				}
				...
				*/
				_addresses [ifr [i].ifr_name] [version] ["address"] = LIB::tools::string (address) + ':' + int_to_addr (address);
				_addresses [ifr [i].ifr_name] [version] ["mask"] = LIB::tools::string (mask) + ':' + int_to_addr (mask);
				_addresses [ifr [i].ifr_name] [version] ["broadcast"] = LIB::tools::string (broadcast) + ':' + int_to_addr (broadcast);
				_addresses [ifr [i].ifr_name] [version] ["network"] = LIB::tools::string (network) + ':' + int_to_addr (network);
			}
		}
