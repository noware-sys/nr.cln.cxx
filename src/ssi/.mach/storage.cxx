...
const noware::var noware::machine::storage::aggregate (const noware::var & result, noware::nr::natural & responses_count, const noware::var & response, const noware::tree <> & expression)
{
	...
	{
		std::cout << "noware::machine::storage::respond()::if::expression[subject]==" << expression ["subject"] << "::in scope" << std::endl;
		std::cout << "noware::machine::storage::respond()::if::expression[subject]==" << expression ["subject"] << "::in scope::reponses_count==[" << responses_count << "]" << std::endl;
		
		if (responses_count > noware::nr::natural (0))
		{
			std::cout << "noware::machine::storage::respond()::if::(responses_count > noware::nr::natural (0))" << "::in scope" << std::endl;
			
			return result && response;
		}
		else
		{
			std::cout << "noware::machine::storage::respond()::if::(responses_count > noware::nr::natural (0))" << "::else::in scope" << std::endl;
...
const bool noware::machine::storage::respond (const noware::tree <> & message, const zyre_event_t * event)
{
	...
	else	// if (message ["type"] == "request")
	{
		...
		else if (message ["subject"] == "removal")
		{
			...
			std::cout << "noware::machine::storage::respond()::message[group]==" << message ["group"] << "" << std::endl;
			std::cout << "noware::machine::storage::respond()::message[key]==" << message ["key"] << "" << std::endl;
			...
			if (!data.exist (message ["group"]))
			{
				std::cout << "noware::machine::storage::respond()::if::(!data.exist (message [group]))==true::in scope" << std::endl;
				response ["value"] = true;
			}
			else
			{
				std::cout << "noware::machine::storage::respond()::if::(!data.exist (message [group]))==false::in scope" << std::endl;
				
				if (!data.get (message ["group"]).exist (message ["key"]))
				{
					std::cout << "noware::machine::storage::respond()::if::(!data.get (message [group]).exist (message [key]))==true::in scope" << std::endl;
					//std::cout << "noware::machine::storage::respond()::if::message[subject]==" << message ["subject"] << "::in scope" << std::endl;
					response ["value"] = true;
				}
				else
...
const bool noware::machine::storage::search_local (noware::var & result, const noware::tree <> & message)
{
	...
	else if (message ["subject"] == "removal")
	{
		...
		*/
		...
		std::cout << "noware::machine::storage::search_local()::message[group]==[" << message ["group"] << ']' << "::pre" << std::endl;
		std::cout << "noware::machine::storage::search_local()::message[key]==[" << message ["key"] << ']' << "::pre" << std::endl;
		/*
		...
		*/
		if (!data.exist (message ["group"])/* && !data [message ["group"]].exist (message ["key"])*/)
		{
			std::cout << "noware::machine::storage::search_local()::if::(!data.exist (message [group]))::in scope" << std::endl;
		...
		{
			std::cout << "noware::machine::storage::search_local()::if::(!data.exist (message [group]))::else::in scope" << std::endl;
			
			//std::cout << "noware::machine::storage::search_local()::message[subject]==[" << message ["subject"] << ']' << "::data [message[\"group\"]==" << message ["group"] << "].exist(message[\"key\"]==" << message ["key"] << ")==true" << std::endl;
			
			if (!data [message ["group"]].exist (message ["key"]))
			{
				std::cout << "noware::machine::storage::search_local():if::(!data [message [\"group\"]].exist (message [\"key\"]))::in scope" << std::endl;
				result = true;
			...
			{
				std::cout << "noware::machine::storage::search_local():if::(!data [message [\"group\"]].exist (message [\"key\"]))::else::in scope" << std::endl;
				result = data [message ["group"]].unset (message ["key"]);
			}
			
			if (data [message ["group"]].empty ())
			{
				std::cout << "noware::machine::storage::search_local():if::data [message [\"group\"]].empty ()::in scope" << std::endl;
				result = result && data.unset (message ["group"]);
			}
			
			std::cout << "noware::machine::storage::search_local():return::result::[" << result << "]" << std::endl;
			//return !result;
			//return result;
			return false;
		}
...
const bool noware::machine::storage::exist (const noware::var & key) const
{
	return exist (noware::container::list::sqlite::group_default, key);
}

const bool noware::machine::storage::exist (const noware::var & group, const noware::var & key) const
{
...
const noware::var noware::machine::storage::get (const noware::var & group, const noware::var & key) const
{
	noware::tree <> expression;
...
const bool noware::machine::storage::set (const noware::var & group, const noware::var & key, const noware::var & value)
{
	noware::tree <> expression;
...
//const noware::nr::natural noware::machine::storage::size (void) const
const noware::nr noware::machine::storage::size (void) const
//const unsigned int noware::machine::storage::size (void) const
{
	noware::tree <> expression;
...
const noware::nr noware::machine::storage::size (const noware::var & group) const
//const unsigned int noware::machine::storage::size (void) const
{
	noware::tree <> expression;
...
*/
const bool noware::machine::storage::remove (const noware::var & group, const noware::var & key)
{
	noware::tree <> expression;
