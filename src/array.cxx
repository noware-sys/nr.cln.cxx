...
//LIB::NAME_A<value, key>::NAME_A (void)
{
	...
	try
	{
		...
		/*
		reset ();
		LIB::nr::natural size = this -> size ();
		
		for (LIB::nr::natural i = 0; i < size; ++ i)
		{
			//archive << current_key ();
...
/*value &*/ const LIB::NAME_A <value, key> & LIB::NAME_A <value, key>::/*operator []*/operator +=/*enqueue*//*add*//*append*/ (const value & value_/*, key & _key_*/)
{
	...
	switch (typeid (key))
	{
		...
		case typeid (LIB::nr::integer):
		case typeid (unsigned long long int):
		case typeid (LIB::nr::natural):
			//ULLONG_MAX
...
/*
	if
		(
			...
			||
			typeid (key) == typeid (LIB::nr::integer)
			||
			typeid (key) == typeid (unsigned long long int)
			||
			* /
			typeid (key) == typeid (LIB::nr::natural)
			||
...
const bool LIB::NAME_A <value, key>::key_numeric (void) const
{
	return
		(
			...
			typeid (key) == typeid (LIB::nr::integer)
			||
			typeid (key) == typeid (LIB::nr::f)
			||
			typeid (key) == typeid (LIB::nr::d)
			||
			typeid (key) == typeid (LIB::nr::rational)
			||
			typeid (key) == typeid (LIB::math::nr)
			*/
			||
			typeid (key) == typeid (LIB::nr)
			||
...
/*
template <typename value, typename key>
const LIB::nr::natural LIB::NAME_A <value, key>::cardinality (void) const
{
	return size ();
}

template <typename value, typename key>
const LIB::nr::natural LIB::NAME_A <value, key>::count (void) const
{
	return size ();
}

template <typename value, typename key>
const LIB::nr::natural LIB::NAME_A <value, key>::magnitude (void) const
{
	return size ();
}

template <typename value, typename key>
const LIB::nr::natural LIB::NAME_A <value, key>::quantity (void) const
{
	return size ();
}
*/
template <typename value, typename key>
//const LIB::nr::natural LIB::NAME_A <value, key>::size (void) const
const LIB::nr LIB::NAME_A <value, key>::size (void) const
{
	return content.size ();
}
...
bool LIB::NAME_A <value, key>::search (const key & _key, container::_value & v) const
{
	...
	LIB::nr::natural n = size ();
	std::list <key>::iterator it;

	//for (LIB::nr::natural index = 0; index < n; ++ index, current = current -> next)
