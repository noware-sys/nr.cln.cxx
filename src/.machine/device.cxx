...
const noware::var noware::machine::device::evaluate (const noware::tree <> & expression, const std::string & group)
{
	...
	if (search_local (result, expression))
		...
		return aggregate (result, multicast (expression, /*noware::machine::device::*/group, ""), expression);
...
const noware::var noware::machine::device::aggregate (const noware::var & result, const noware::var & response, const noware::tree <> & expression)
{
	return response;
}

const noware::var noware::machine::device::multicast (const noware::tree <> & message, const std::string & group, const std::string & filter)
{
...
//		filter += (noware::var) '/' + message ["name"];
	...
	return receive_local (group, filter);
...
const noware::var noware::machine::device::receive_local (const std::string & group, /*const std::string & response_type, */const std::string & filter) const
{
  ...
  std::cout << "noware::machine::device::receive_local()::pre-loop" << std::endl;
	for (noware::number::natural n = 0; n < /*zlist_size (peers)*/ node.peers_count (group); ++n)
	{
		std::cout << "noware::machine::device::receive_local()::while::n==[" << n << ']' << std::endl;
		...
		*/
		...
		// search() returns bool if we have found the (correct) answer that we were searching for.
