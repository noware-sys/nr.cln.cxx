...
static const std::string LIB::memory::default_database_name;
...
const std::string LIB::memory::default_group;
const std::string LIB::memory::default_name;
const std::string LIB::memory::default_value;

const bool LIB::memory::initialize (const LIB::database::sqlite *& _db, const LIB::memory::key & grp, const LIB::memory::key & previous_grp, const LIB::memory::key & nm, const std::string & _database_name)
{
	database_name = _database_name;
	
	if (_db == NULL)
	{
		db = new LIB::database::sqlite (database_name);
		...
		arguments [1] = default_group;
		arguments [2] = 0;
		arguments [3] = default_name;
		arguments [4] = 0;
		
		db -> query ("INSERT OR IGNORE INTO \"Entities\" (\"Group\", \"Name: Reference\", \"Name\", \"Content: Reference\") VALUES (?, ?, ?, ?)", arguments);
	}
	else
	{
		db = _db;
...
const bool LIB::memory::initialize (const std::string & _database_name)
{
	initialize (NULL, default_group, default_previous_group, default_name, _database_name);
}

LIB::memory::memory (const LIB::database::sqlite *& _db, const LIB::memory::key & grp, const LIB::memory::key & previous_grp, const LIB::memory::key & nm, const std::string & _database_name)
{
	initialize (const LIB::database::sqlite *& _db, const LIB::memory::key & grp, const LIB::memory::key & previous_grp, const LIB::memory::key & nm, _database_name);
}

LIB::memory::memory (const std::string & _database_name)
{
//	db = other.db;
//	group = other.group;
...
const LIB::memory::relation LIB::memory::relate (const LIB::memory::memory & other) const
{
...
{
	//typeid ();
	return db == other.db;	// Is this all?
}
...
const LIB::memory & LIB::memory::operator = (const LIB::memory & other)
{
	if (db != other.db)
	{
		...
		// We need to copy over other's complete tree of its currently pointed to entity
		// to the our currently referred one, making our currently referred one point to our new copy.
	...
	else if (group != other.group)
	{
		...
		// but it is referring to another entity;
		// so, link to that other entity from the one currently pointed to (remember that "this == other", logically).
		
		if (clear ())
		{
			switch (relate (other))
			{
				case relation::sibbling:
					// Set the value of the current "Name" to the other's "Name".
					
					LIB::NAME_A <LIB::NAME_V, LIB::NAME_V> arguments;
					LIB::NAME_A <LIB::NAME_A <LIB::NAME_V, LIB::NAME_V>, LIB::NAME_V> result;
					
					arguments [1] = other.group;
					arguments [2] = other.previous_name;
					
					if (other.db -> query (result, "SELECT \"Content: Reference\", \"Content\" FROM \"Entities\" WHERE \"Group\" = ? AND \"Name\" = ?", arguments))
					{
						// Use the same variable.
						arguments.clear ();
						
						arguments [1] = result [1] [1];
						arguments [2] = result [2] [1];
						arguments [3] = group;
						arguments [4] = previous_name;
						
						if (db -> ("UPDATE \"Entities\" SET \"Content: Reference\" = ?, \"Content\" = ? WHERE \"Group\" = ? AND \"Name\" = ?"))
						{
							// Only "other" should be cleared.
							// No referenced entity should be cleared in this call,
							// because we have just made a copy.
							other.clear ();
						}
					}
...
const LIB::memory & LIB::memory::operator = (const LIB::memory::value & val)
{
	if (clear ())
	{
		...
		db -> query ("INSERT OR REPLACE INTO \"Entities\" (\"Group\", \"Name: Reference\", \"Name\", \"Content: Reference\", \"Content\") VALUES (?, ?, ?, ?, ?)", arguments);
	}
...
const LIB::mathematics::numbers::natural LIB::memory::size (const bool & recursive, const bool & inclusive) const
{
	...
	else
	{
		...
		{
			{
				memory mem (db, element.v, group, element.k, database_name);
...
const bool LIB::memory::clear (void) const
{
	...
	else
	{
		...
		if (db -> query (NULL, "UPDATE \"Entities\" SET \"Content: Reference\" = ?, \"Content\" = ? WHERE \"Group\" = ? AND \"Name\" = ?", arguments))
		{
			db -> query (NULL, "ROLLBACK", NULL);
			return false;
		}
...
const LIB::memory LIB::memory::get (const LIB::memory::key & nm) const
{
	...
	*/
	...
	if (db -> query (result, "\
			SELECT \"Content\"\n\
			FROM \"Entities\"\n\
			WHERE \"Group\" = ? AND \"Name\" = ? AND \"Content: Reference\" != 0;\
			", arguments))
	{
		if (! result.empty ())
		{
...
const bool LIB::memory::set/*_literal*/ (const LIB::memory::value & val)
{
	...
	return db -> query (NULL, "\
			BEGIN;\n\
			UPDATE \"Entities\"\n\
			SET \"Content: Reference\" = 0, \"Content\" = ?\n\
			WHERE \"Group\" = ? AND \"Name\" = ?;\n\
			COMMIT;\
			", arguments);
}
...
const bool LIB::memory::set_group (const LIB::memory::key & name, const LIB::memory::key & used_group)
{
	...
	if (!
		db -> query (NULL, "\
			...
			(\"Group\", \"Name\", \"Content: Reference\", \"Content\")\n\
			VALUES (?, ?, ?, NULL)\
			", arguments)
	)
	{
...
const bool LIB::memory::set_group (void)
{
	...
	return db -> query (NULL, "\
			BEGIN;\n\
			UPDATE \"Entities\"\n\
			SET \"Content: Reference\" = ?, \"Content\" = NULL\n\
			WHERE \"Group\" = ? AND \"Name\" = ?;\n\
			COMMIT;\
			", arguments);
}
