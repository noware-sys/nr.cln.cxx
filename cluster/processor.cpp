...
LIB::cluster::processor::instruction LIB::cluster::processor::pop (void)
{
	LIB::cluster::processor::instruction _instruction;	// Fallback variable.
	LIB::NAME_V instruction_key = 1, instruction_subkey = 1;

	if (memory.search ("instruction_queue_vacant"))
	{
		while (memory.get ("instruction_queue_vacant") != "true")
		{
			// Wait.
		}
	}

	if (! memory.set ("instruction_queue_vacant", "false"))
		return _instruction;

	if (memory.search ((std::string) instruction_key))
	{
		_instruction = memory.get (instruction_key);

		assert (memory.unset (instruction_key));
	...
	if (memory.search ((std::string) instruction_key + "_" + instruction_subkey))
	{
		assert (memory.rename ((std::string) instruction_key + "_" + instruction_subkey, instruction_key));

		//_instruction = memory.get (instruction_key);

		while (memory.search ((std::string) instruction_key + "_" + (++ instruction_subkey)))
		{
			assert (memory.rename ((std::string) instruction_key + "_" + instruction_subkey, (std::string) instruction_key + "_" + (instruction_subkey - 1)));
		}
	}
	else
	{
		while (memory.search (instruction_key))
		{
			assert (memory.rename (instruction_key, instruction_key - 1));

			instruction_subkey = 1;

			while (memory.search ((std::string) instruction_key + "_" + instruction_subkey))
			{
				assert (memory.rename ((std::string) instruction_key + "_" + instruction_subkey, (std::string) (instruction_key - 1) + "_" + instruction_subkey));
	...
	memory.set ("instruction_queue_vacant", "true");	// Release the lock.
...
bool LIB::cluster::processor::enqueue (LIB::cluster::processor::instructions _instructions)
{
	...
	if (! memory.set (instruction_key, _instructions._instructions.front ()))
		return false;
	...
	while (_instructions._instructions.size () > 0)
	{
		assert (memory.set ((std::string) instruction_key + "_" + instruction_subkey, _instructions._instructions.front ()));

		_instructions._instructions.pop ();
		++ instruction_subkey;
	}

	return memory.set ("instruction_queue_vacant", "true");
}

bool LIB::cluster::processor::enqueue (LIB::cluster::processor::instruction _instruction)
{
	LIB::NAME_V instruction_key = 1;
	...
	memory.set (instruction_key, _instruction);
